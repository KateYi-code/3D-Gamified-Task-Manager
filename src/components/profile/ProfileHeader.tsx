...
export default function ProfileHeader({ id }: Props) {
  ...
  if (isLoading || !profile) {
    return <div className="text-center text-muted-foreground py-6">Loading...</div>;
  }
  ...
  return (
    <div className="w-full max-w-5xl mx-auto flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-8">
      <Avatar className="w-24 h-24 mx-auto sm:mx-0">
        ...
        <AvatarFallback>
          <TextAvatar name={profile.name ?? profile.email} size={"large"} />
        </AvatarFallback>
      </Avatar>
      ...
      <div className="flex-1 space-y-2 text-center sm:text-left">
        <h1 className="text-2xl font-bold">
          ...
          <span className="ml-2 text-sm text-muted-foreground font-normal">@{profile.id}</span>
        </h1>
        <p className="text-muted-foreground text-sm">
          👥 {profile.followersCount} Followers ｜ 👣 {profile.followingCount} Following ｜ ❤️{" "}
      ...
      <div className="flex justify-center sm:justify-end gap-3 flex-wrap">
        ...
        {isSelf ? (
          ...
          >
            {followMutation.isPending ? "Processing..." : isFollowing ? "Unfollow" : "Follow"}
          </Button>
        )}
      </div>
      {isMe && profile && (
